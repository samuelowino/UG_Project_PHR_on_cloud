...
public class DocLog extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String user=request.getParameter("name_");
        String pass=request.getParameter("password");
           try{
                    if (user.equalsIgnoreCase("doc")&&(pass.equalsIgnoreCase("doc"))) {
                HttpSession session=request.getSession();
                session.setAttribute("userid",request.getParameter("name_") );
                System.out.println(">>>>>>>"+session.getAttribute("userid"));
                response.sendRedirect("DocHome.jsp");
            } else {
                response.sendRedirect("DocLogin.jsp?msg=Check Userid or Password");
            }
        }catch(Exception e){
            e.printStackTrace();
        }
